# my-packer-image-builds jeffs-virtualbox-image-debian
...
# ----------------------------------------
# Source Image (ISO)
# Debian 13 (Trixie) netinst — small, downloads packages during install.
# Check the latest point release at https://www.debian.org/CD/netinst/
...
source "virtualbox-iso" "jeffs-debian" {
  ...
  # instead of the default IDE for the same reasons as the Ubuntu build.
  ...
  #          serving the http/ folder (preseed.cfg)
  #          Debian installer fetches preseed.cfg from this server
  # ---------------------------------------------------------------
  ...
  #          the auto preseed install and point it at the HTTP server
  # ---------------------------------------------------------------

  # Debian 13 (Trixie) installer boot prompt.
  # Note: kernel path is /install.amd/vmlinuz (not /install/vmlinuz like Debian 11/12).
  boot_wait = "10s"
  boot_command = [
  ...
  #          and exports it as an OVF appliance to output_directory.
  #          Use convert-ovf-to-vbox.sh to import it back into the
  #          VirtualBox Manager UI.
  # ---------------------------------------------------------------
...
build {
  ...
  sources = ["source.virtualbox-iso.jeffs-debian"]
  ...
  #          VM exports as OVF — use convert-ovf-to-vbox.sh
  # ---------------------------------------------------------------
